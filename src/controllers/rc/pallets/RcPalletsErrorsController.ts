...
export default class RcPalletsErrorsController extends AbstractController<PalletsErrorsService> {
	static controllerName = 'RcPalletsErrors';
	...
	constructor(_api: string) {
		...
		super(rcSpecName, '/rc/pallets/:palletId/errors', new PalletsErrorsService(rcSpecName));
		this.initRoutes();
	}
	...
	private getErrorById: RequestHandler<IPalletsErrorsParam, unknown, unknown> = async (
		...
		RcPalletsErrorsController.sanitizedSend(res, result);
	};
