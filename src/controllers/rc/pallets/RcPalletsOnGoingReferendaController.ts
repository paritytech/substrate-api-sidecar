...
export default class RcPalletsOnGoingReferendaController extends AbstractController<PalletsOnGoingReferendaService> {
	static controllerName = 'RcPalletsOnGoingReferenda';
	...
	constructor(_api: string) {
		...
		super(rcSpecName, '/rc/pallets/on-going-referenda', new PalletsOnGoingReferendaService(rcSpecName));
		this.initRoutes();
	}
	...
	private getPalletOnGoingReferenda: RequestHandler = async ({ query: { at } }, res): Promise<void> => {
		...
		RcPalletsOnGoingReferendaController.sanitizedSend(res, result);
	};
}
