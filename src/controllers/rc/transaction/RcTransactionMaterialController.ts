...
import { MetadataOpts } from '../../transaction/TransactionMaterialController';
...
export default class RcTransactionMaterialController extends AbstractController<TransactionMaterialService> {
	static controllerName = 'RcTransactionMaterial';
	...
	constructor(_api: string) {
		...
		super(rcSpecName, '/rc/transaction/material', new TransactionMaterialService(rcSpecName));
		this.initRoutes();
	}
	...
	private getTransactionMaterial: RequestHandler = async ({ query: { at, metadata } }, res): Promise<void> => {
		...
		RcTransactionMaterialController.sanitizedSend(
			res,
			await this.service.fetchTransactionMaterial(rcApi, hash, metadataArg),
		);
	};
	...
	private getTransactionMaterialwithVersionedMetadata: RequestHandler = async (
		...
		RcTransactionMaterialController.sanitizedSend(
			res,
			await this.service.fetchTransactionMaterialwithVersionedMetadata(rcApi, api, hash, metadataArg, version),
		);
	};
}
